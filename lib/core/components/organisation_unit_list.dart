...
class _OrganisationUnitListState extends State<OrganisationUnitList> {
  ...
  void initState() {
    super.initState();
    setState(() {
      hasChildren = widget.organisationUnit.children!.isNotEmpty;
    });
  }
  ...
  void onSelectOrganisationUnit(
    ...
    if (widget.allowedSelectedLevels!.isEmpty ||
        widget.allowedSelectedLevels!.contains(organisationUnit.level)) {
  ...
  Widget build(BuildContext context) {
    return Container(
      margin: const EdgeInsets.only(top: 3.0, left: 10.0, bottom: 3.0),
      child: Column(
        children: [
          Row(
            children: [
              ...
              Expanded(
                  child: InkWell(
                onTap: () =>
                    onSelectOrganisationUnit(context, widget.organisationUnit),
                child: Container(
